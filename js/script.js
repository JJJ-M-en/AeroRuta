...
async function inicializarTableroSalidas() {
  ...
  try {
    ...
    tablero.innerHTML =
      "<p class=\"flap-row\" style=\"opacity:1;\">No se pudo conectar con la API (" + error.message +
      "). Verifica que el servidor esté corriendo en " + API_BASE_URL + ".</p>";
  }
}
...
function inicializarVuelos() {
  ...
  lista.addEventListener("click", (evento) => {
    ...
    document.getElementById("resumenVuelo").innerHTML =
      "<span class=\"codigo\">" + vuelo.codigo + "</span> · " + vuelo.origen + " → " + vuelo.destino +
      " · $" + Number(vuelo.precio).toLocaleString("es-CO") + " por pasajero";
    modalReservar.show();
  });
  ...
  document.getElementById("formReservar").addEventListener("submit", async (evento) => {
    ...
    try {
      ...
      await render();
      mostrarAlerta(
        "Reserva confirmada para <strong>" + nombrePasajero + "</strong> — vuelo " + reserva.codigoVuelo +
        ", asiento " + reserva.asiento + " (" + CLASE_TEXTO[reserva.clase] + ").",
        "success"
      );
    } catch (error) {
    ...
    try {
      ...
      reservas.forEach((reserva) => {
        ...
        item.innerHTML =
          ...
          "<span class=\"fecha-reserva\">" + reserva.nombrePasajero + " · " + reserva.cantidadPasajeros +
          " pasajero(s) · " + CLASE_TEXTO[reserva.clase] + " · viaja " + reserva.fechaViaje + "</span>" +
          "</span>" +
